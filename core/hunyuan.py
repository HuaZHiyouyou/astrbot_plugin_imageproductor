...
请直接返回英文提示词，不要有任何其他内容。"""
        ...
        url = f"{api_url}/hunyuan/v1/chat/completions"
        headers = {
            "Authorization": f"Bearer {api_key}",
            "Content-Type": "application/json"
        }

        logger.info(f"[ImageProducer] 正在调用混元 Chat API...")
    ...
    async def _analyze_reference_images(
        ...
        try:
            url = f"{api_url}/hunyuan-vision/chat"
            headers = {
                "Authorization": f"Bearer {api_key}",
                "Content-Type": "application/json"
            }
...
只返回提示词文本，不要返回其他内容。提示词应该足够详细以便用于图像生成。"""
        ...
        **kwargs
        ...
        try:
            ...
            if has_images and auto_switch_mode:
                ...
                vision_model = model or self.config.get("vision_model", "hunyuan-vision")
    ...
    async def test_connection(
        ...
        try:
            ...
            url = f"{api_url}/hunyuan/v1/models"
            headers = {
                "Authorization": f"Bearer {api_key}",
            }
