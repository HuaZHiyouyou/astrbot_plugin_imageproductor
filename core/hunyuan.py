"""
腾讯混元 图像生成 Provider (整合普通生成和Chat模式)
支持：
...
class HunyuanProvider(BaseProvider):
    """腾讯混元 图像生成提供商 (支持普通模式和Chat模式自动切换)"""
    ...
    async def _generate_with_chat_api(
        ...
        **kwargs
        ...
        if image_b64_list and len(image_b64_list) > 0:
            logger.info(f"[ImageProducer] 混元Chat检测到 {len(image_b64_list)} 张参考图片")
...
请直接返回英文提示词，不要有任何其他内容。"""
        ...
        url = self._build_url(api_url, "/hunyuan/v1/chat/completions")
        headers = {
            "Authorization": f"Bearer {api_key}",
            "Content-Type": "application/json"
        }

        logger.info(f"[ImageProducer] 正在调用混元 Chat API: {url}, model: {model}")

        async with self.session.post(url, headers=headers, json=payload) as response:
            if response.status == 200:
                result = await response.json()
                logger.info(f"[ImageProducer] 混元 Chat API返回: {result}")
                ...
                if "choices" in result and len(result["choices"]) > 0:
                    ...
                    if match:
                        ...
                        if img_src.startswith("data:image/"):
                            ...
                            logger.info(f"[ImageProducer] 成功从混元Chat响应中提取图片")
                            return ImageResult(success=True, b64_json=b64_data)
                        else:
                            logger.info(f"[ImageProducer] 从混元Chat响应获取到URL图片")
                            return ImageResult(success=True, image_url=img_src)

                    # 未匹配到 markdown 格式，检查是否是纯 base64 数据
                    if self._is_likely_base64(content):
                        logger.info(f"[ImageProducer] 混元 Chat API 返回纯 base64 数据，长度: {len(content)}")
                        return ImageResult(success=True, b64_json=content)

                    logger.warning(f"[ImageProducer] 混元Chat未返回图片")
                    return ImageResult(success=False, error=f"混元Chat未返回图片")

                return ImageResult(success=False, error="混元 Chat API返回格式异常")
            else:
                error_text = await response.text()
                logger.error(f"[ImageProducer] 混元 Chat API错误: {response.status} - {error_text}")
    ...
    async def _analyze_reference_images(
        ...
        """使用混元视觉分析参考图片，生成详细提示词"""
        try:
            url = self._build_url(api_url, "/hunyuan-vision/chat")
            headers = {
                "Authorization": f"Bearer {api_key}",
                "Content-Type": "application/json"
            }
            logger.info(f"[ImageProducer] 混元分析参考图片 API: {url}")
...
Only return the prompt text, nothing else. The prompt should be 150-300 words, detailed enough for image generation."""
            ...
            from astrbot.api import logger
            logger.warning(f"[ImageProducer] 混元分析参考图片失败: {e}")
            return original_prompt
    ...
    async def _generate_with_images_api(
        ...
        **kwargs
        ...
        if is_multimodal and image_b64_list and len(image_b64_list) > 0:
            ...
            # 传统文生图模型：使用图像生成端点
            url = self._build_url(api_url, "/hunyuan/v1/images/generations")
            payload = {
                "model": model,
                "prompt": prompt,
                "n": 1,
            ...
            logger.info(f"[ImageProducer] 混元文生图 API: {url}, model: {model}")
                ...
                if "choices" in result and len(result["choices"]) > 0:
                    ...
                    if content:
                        ...
                        if self._is_likely_base64(content):
                            ...
                            logger.info(f"[ImageProducer] 混元 Chat API 返回文本内容，长度: {len(content)}")
    ...
    async def generate_image(
        ...
        **kwargs
        ...
        try:
            ...
            gen_model = model or self.config.get("model", "hunyuan-image")
            ...
            logger.error(f"[ImageProducer] 混元生成图片失败: {e}", exc_info=True)
    ...
    async def test_connection(
        ...
        try:
            ...
            url = self._build_url(api_url, "/hunyuan/v1/models")
            headers = {
                "Authorization": f"Bearer {api_key}",
            }
            logger.info(f"[ImageProducer] 混元测试连接 API: {url}")
