"""
DeepSeek Vision 图像生成提供商
使用 DeepSeek 的视觉理解能力生成图像
"""
import re
...
class DeepSeekVisionProvider(BaseProvider):
    """DeepSeek Vision 提供商"""
        ...
        **kwargs
        ...
        try:
            ...
            if image_b64_list and len(image_b64_list) > 0:
                logger.info(f"[ImageProducer] DeepSeek Vision 检测到 {len(image_b64_list)} 张参考图片")
...
请直接返回英文提示词，不要有任何其他内容。"""
            ...
            async with self.session.post(url, headers=headers, json=payload) as response:
                if response.status == 200:
                    ...
                    logger.error(f"[ImageProducer] DeepSeek Vision API错误: {response.status} - {error_text}")
                    return ImageResult(success=False, error=f"API错误: {response.status}")

        except Exception as e:
            logger.error(f"[ImageProducer] DeepSeek Vision生成异常: {e}", exc_info=True)
