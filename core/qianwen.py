"""
阿里云千问 图像生成 Provider (整合普通生成和Chat模式)
支持：
...
class QianwenProvider(BaseProvider):
    """阿里云千问 图像生成提供商 (支持普通模式和Chat模式自动切换)"""
    ...
    async def _generate_with_chat_api(
        ...
        **kwargs
        ...
        if image_b64_list and len(image_b64_list) > 0:
            logger.info(f"[ImageProducer] 千问Chat检测到 {len(image_b64_list)} 张参考图片")
...
请直接返回英文提示词，不要有任何其他内容。"""
        ...
        async with self.session.post(url, headers=headers, json=payload) as response:
            if response.status == 200:
                ...
                logger.info(f"[ImageProducer] 千问 Chat API返回: {result}")
                ...
                if "choices" in result and len(result["choices"]) > 0:
                    ...
                    if match:
                        ...
                        if img_src.startswith("data:image/"):
                            ...
                            logger.info(f"[ImageProducer] 成功从千问Chat响应中提取图片")
                ...
                return ImageResult(success=False, error="千问 Chat API返回格式异常")
            else:
                error_text = await response.text()
                logger.error(f"[ImageProducer] 千问 Chat API错误: {response.status} - {error_text}")
...
只返回提示词文本，不要返回其他内容。提示词应该足够详细以便用于图像生成。"""
            ...
            from astrbot.api import logger
            logger.warning(f"[ImageProducer] 千问分析参考图片失败: {e}")
            return original_prompt
        ...
        **kwargs
        ...
        try:
            ...
            if has_images and auto_switch_mode:
                ...
                gen_model = model or self.config.get("model", "qwen-vl-plus")
                return await self._generate_with_images_api(
                    prompt, gen_model, size, api_key, api_url, image_b64_list, **kwargs
                )

        except Exception as e:
            logger.error(f"[ImageProducer] 千问生成图片失败: {e}", exc_info=True)
