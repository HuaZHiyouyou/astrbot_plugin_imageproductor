...
请直接返回英文提示词，不要有任何其他内容。"""
    ...
    async def _analyze_reference_images(
        ...
        try:
            ...
            if use_chinese:
                analysis_text = f"""请分析这 {image_count} 张参考图片并生成详细的图像生成提示词。

用户需求：{original_prompt}

基于所有参考图片，创建一个详细的提示词：
1. 描述每张参考图片的视觉风格、构图和氛围
2. 识别所有参考图片的共同风格特征
3. 保留参考图片的关键视觉元素
4. 融入用户需求：{original_prompt}
5. 仅返回提示词文本，150-300字
6. 包含：主体、环境、艺术风格、光线、色彩、构图、相机角度、质量关键词

只返回最终提示词，不要其他内容。"""
            else:
                analysis_text = f"""Please analyze these {image_count} reference image(s) and create a detailed image generation prompt.

The user wants to generate: {original_prompt}

Based on ALL reference images, create a detailed prompt that:
1. Describes the visual style, composition, and mood from each reference
...
5. Returns ONLY the prompt text, 150-300 words
6. Include: subject, environment, art style, lighting, colors, composition, camera angle, quality keywords

Only return the final prompt text, nothing else."""
    ...
    async def generate_image(
        ...
        **kwargs
        ...
        """调用 OpenAI API 生成图像
        智能模式：
        1. 多模态模型（如 GPT-Image-1）：直接传入图片+文字
        ...
        try:
            ...
            if is_multimodal and has_images:
                ...
                api_key, api_url = self._get_api_config(use_vision=False)
                return await self._generate_with_images_api(
                    enhanced_prompt, gen_model, size, api_key, api_url, image_b64_list,
                    quality=quality, style=style, **kwargs
                )
            else:
